...
int main(int argc,char**argv){
 ...
 titanEffect(command,48,21,0,50,1000);assert(std::string(command)=="CHNL 3;vibrate 1000 0.050 160 0;\r");assert(!titanEffect(command,48,20,3,50,8));assert(!titanEffect(command,48,20,0,50,3001));assert(titanFrequency(2)==130);
 ...
 run(200000);assert(titan.commands.size()==1&&titan.commands[0]=="CHNL 1;vibrate 95 1.000 3000 1 0;\r"&&sent[1]==1000);
 cmd("D3 1 HELLO 0");assert(response("BUSY"));
 run(3000000);assert(titan.commands.size()==2&&titan.commands[1]=="CHNL 3;vibrate 160 1.000 3000 1 0;\r"&&!sent[1]&&sent[0]==1000);
 run(3000000);assert(titan.commands.size()==3&&titan.commands[2]=="CHNL 2;vibrate 130 1.000 3000 1 0;\r"&&!sent[0]&&sent[2]==1000);
